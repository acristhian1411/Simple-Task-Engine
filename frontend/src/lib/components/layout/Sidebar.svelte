<script>
  ...
  const defaultUser = {
    ...
    avatar:
      "https://lh3.googleusercontent.com/aida-public/AB6AXuBfj0AMzX5L2kvHqL3L-WDYAhtVnM9_7yZg_mm2HEPv6tezHc2M19Hjpe-v6zKeDBuWbfqtyzc8GHEbjBedXp5OzSH-nz3_dx4bRMaYFA3Ni74B8EXDc7y6TeHf6N385OIuaULnuQRj64iDbDNBbDGUW1YU9i0w3DWMegVeDJ5zyOOWyLQejY8lOxnRW5haUEZrmynKNHSwjU54zVmSZowIDPQMjunaJAhGhR24CCqb7nDiT8RmScTOpX-6ftp_qoYspDmS85eo6-I",
  };
  ...
  <div class="p-6 pb-2">
    ...
    <div class="flex gap-3 items-center mb-8">
      ...
      <div class="flex flex-col">
        <h1
          class="text-gray-900 dark:text-white text-base font-bold leading-none"
        >
          Maius Kanban
        </h1>
        <p class="text-gray-600 dark:text-gray-400 text-xs font-medium mt-1">
          Gestión Pro
        </p>
      </div>
    </div>
    ...
    <div class="flex flex-col gap-1">
      {#each navItems as item}
        ...
        >
          <span
            class="material-symbols-outlined text-[24px] {activeItem === item.id
              ? ''
              : 'group-hover:text-indigo-600 transition-colors'}"
            >{item.icon}</span
          >
          <p
            class="text-sm font-{activeItem === item.id
              ? 'bold'
    ...
    <div class="border-t border-gray-200 dark:border-gray-700 pt-4">
      ...
      >
        ...
        <div class="flex flex-col min-w-0 flex-1">
          <p
            class="text-gray-900 dark:text-white text-sm font-medium leading-none truncate"
          >
            {currentUser.name}
          </p>
          <p class="text-gray-600 dark:text-gray-400 text-xs mt-1 truncate">
            {currentUser.email}
          </p>
        </div>
      </button>
