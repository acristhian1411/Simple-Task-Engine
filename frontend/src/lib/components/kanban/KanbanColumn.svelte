<script>
  ...
  function getHeaderClasses() {
    const base = 'p-4 flex items-center justify-between border-b backdrop-blur-sm rounded-t-xl sticky top-0 z-10';
    const statusClasses = {
      default: 'border-slate-200 dark:border-surface-border/50 bg-white/60 dark:bg-surface-dark',
      active: 'border-slate-200 dark:border-surface-border/50 bg-white/60 dark:bg-surface-dark ring-1 ring-primary/20 dark:ring-primary/10',
      blocked: 'border-red-100 dark:border-red-900/20 bg-red-50 dark:bg-red-950/5',
      completed: 'border-slate-200 dark:border-surface-border/50 bg-white/60 dark:bg-surface-dark'
    };
  ...
  function getColumnClasses() {
    ...
    const statusClasses = {
      default: 'bg-slate-50 dark:bg-surface-dark border border-slate-200 dark:border-surface-border/50',
      active: 'bg-slate-100 dark:bg-surface-dark border-t-4 border-t-primary border-x border-b border-x-slate-200 dark:border-x-surface-border/50 border-b-slate-200 dark:border-b-surface-border/50',
      blocked: 'bg-red-50 dark:bg-red-950/10 border-t-4 border-t-red-500 border-x border-b border-x-red-100 dark:border-x-red-900/20 border-b-red-100 dark:border-b-red-900/20',
      completed: 'bg-slate-100 dark:bg-surface-dark border border-slate-200 dark:border-surface-border/50 opacity-80 hover:opacity-100 transition-opacity'
    };
    return `${base} ${statusClasses[status]}`;
  }
  ...
  function getBadgeClasses() {
    ...
    const colorClasses = {
      default: 'bg-slate-200 dark:bg-[#324d67] text-slate-600 dark:text-slate-300',
